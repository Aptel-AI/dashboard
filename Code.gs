...
function writeAddSale(body, ss, officeId) {
  ...
  var dupRow = _findDuplicateSale(sheet, email, String(body.campaign || '').trim(), body);
  if (dupRow > 0) {
    var dupWebhook = '';
    try { dupWebhook = _fireWebhook(body, units, teamEmoji); } catch (e) { dupWebhook = 'CATCH: ' + e.message; }
    return {
      ok: true,
      duplicate: true,
      rowIndex: dupRow,
      units: units,
      yeses: yeses,
      webhookDebug: dupWebhook || 'no-return'
    };
  }
