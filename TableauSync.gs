...
var REPORTS = {
  ...
  'b2b-order-log': {
    ...
    dateFilterField: 'Order Date',       // Tableau API hint (may not work for ranges)
    dateRangeDays: 30,
    dateFilterColumn: 'sp.Order Date (copy)',  // Client-side date filter (reliable)
 ...
 */
function _findViewId(config, token, siteId, viewContentUrl) {
...
/**
 * Download view data as CSV, with optional date filter
 */
function _downloadViewData(config, token, siteId, viewId, dateFilterField, dateRangeDays) {
  var url = config.server + '/api/' + TABLEAU_API_VERSION
    + '/sites/' + siteId + '/views/' + viewId + '/data';

  // Add date filter if specified
  if (dateFilterField && dateRangeDays) {
    ...
    url += '?vf_' + encodeURIComponent(dateFilterField) + '=' + encodeURIComponent(startStr + ',' + endStr);
  }
...
function syncReport(reportKey) {
  ...
  try {
    // Find view
    var viewId = _findViewId(config, auth.token, auth.siteId, report.viewContentUrl);
    Logger.log('Found view: ' + viewId);

    // Download data
    var csv = _downloadViewData(config, auth.token, auth.siteId, viewId,
      report.dateFilterField, report.dateRangeDays);
    Logger.log('Downloaded CSV: ' + csv.length + ' chars');

    // Parse and filter
    var data = _parseCsv(csv);
    Logger.log('Parsed ' + data.length + ' rows (including header)');
    ...
    // Client-side date filter (reliable — Tableau API vf_ is discrete, not range)
...
/**
 * Log all column headers from a Tableau view (run from editor to find exact column names)
 */
function logColumnHeaders() {
  var config = _getConfig();
  var auth = _tableauSignIn(config);
  try {
    var viewId = _findViewId(config, auth.token, auth.siteId, 'ATTTRACKER-B2B/sheets/ORDERLOG');
    var csv = _downloadViewData(config, auth.token, auth.siteId, viewId, null, null);
    var data = _parseCsv(csv);
    var headers = data[0];
    Logger.log('=== ALL ' + headers.length + ' COLUMN HEADERS ===');
