...
function _extractIndeedTable(sheet) {
  ...
  for (var r = tableStartRow; r < Math.min(tableStartRow + 15, data.length); r++) {
    ...
    for (var k = 0; k < ROW_DEFS.length; k++) {
      var matched = false;
      for (var p = 0; p < ROW_DEFS[k].patterns.length; p++) {
        if (label.indexOf(ROW_DEFS[k].patterns[p]) >= 0) { matched = true; break; }
      }
      if (matched) {
        var key = ROW_DEFS[k].key;
        if (localCol >= 0) localData[key] = _parseCurrencyVal(data[r][localCol]);
        if (nlrCol >= 0)   nlrData[key]   = _parseCurrencyVal(data[r][nlrCol]);
        if (totalCol >= 0) totalData[key]  = _parseCurrencyVal(data[r][totalCol]);
        break;
      }
    }
  }
  ...
  if (!Object.keys(totalData).length && !Object.keys(localData).length) return null;

  return { local: localData, nlr: nlrData, total: totalData };
}

/** Strip dollar signs, commas, and whitespace from currency values → number */
function _parseCurrencyVal(v) {
  if (v === null || v === undefined || v === '') return 0;
